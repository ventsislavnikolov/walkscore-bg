...
export function NearbyAmenities({ amenities, onSelect }: NearbyAmenitiesProps) {
  ...
  const grouped = amenities.reduce<Record<string, Amenity[]>>(
    (acc, amenity) => {
  ...
  return (
    <div className="space-y-5">
      ...
      {Object.entries(grouped).map(([category, items]) => {
        ...
        return (
          <div key={category}>
            <div className="mb-2 flex items-center gap-2 font-medium text-sm text-stone-700">
              <span>{meta.icon}</span>
              ...
              {items.slice(0, 5).map((amenity) => (
                ...
                >
                  <span className="truncate pr-3 text-stone-700">
                    {(locale === "bg" ? amenity.nameBg : amenity.name) ||
                      amenity.name ||
                      amenity.nameBg}
                  </span>
