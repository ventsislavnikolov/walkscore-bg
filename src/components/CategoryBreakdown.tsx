  ...
  return (
    <div className="space-y-3 rounded-3xl bg-white p-5 shadow-[0_24px_60px_-40px_rgba(68,64,60,0.45)] ring-1 ring-stone-200/70">
      {Object.entries(components).map(([key, score]) => {
        ...
        return (
          <div key={key} className="grid grid-cols-[2rem_1fr_5.5rem_2rem] items-center gap-3">
            <span className="text-center text-lg">{meta.icon}</span>
            <span className="text-sm text-stone-700">{locale === 'bg' ? meta.bg : meta.en}</span>
            ...
            <span className="text-right text-sm font-semibold text-stone-600">{roundedScore}</span>
          </div>
        )
      })}
    </div>
