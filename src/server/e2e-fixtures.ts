...
export function getE2EScoreFixtureByCoords(lat: number, lng: number): ScoreResult {
...
export function getE2EAmenitiesFixture(score: ScoreResult): Amenity[] {
  return (
    E2E_LOCATIONS.find((location) => location.score.address === score.address)?.amenities ??
    buildAmenities(score.lat, score.lng)
  )
}
