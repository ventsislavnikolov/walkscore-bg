...
test.describe('Home Page', () => {
  ...
  test('quick links navigate to score pages', async ({ page }) => {
    ...
    await waitForHydration(page)
    await expect(page.getByRole('heading', { level: 1 })).toContainText('ul. Graf Ignatiev 24, Sofia')
  })
  ...
  test('search submit navigates to a score page', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/score\?address=/)
    await waitForHydration(page)
    await expect(page.getByRole('heading', { level: 1 })).toContainText('Lozenets, Sofia')
  })
})
