...
test.describe("Compare Page", () => {
  test("compares two Sofia addresses and surfaces a winner", async ({
    page,
  }) => {
    ...
    await waitForHydration(page);
    await expect(
      page.getByRole("heading", { level: 2, name: "Lozenets, Sofia" })
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { level: 2, name: "Lyulin, Sofia" })
    ).toBeVisible();
